		...
		if (subtype == STYPE_Selector) {
			...
			if (result.size() > 0)
			{
				...
				if (Name == "Xiaomi Wireless Switch") {
					...
					m_sql.SetDeviceOptions(atoi(Idx.c_str()), m_sql.BuildDeviceOptions("SelectorStyle:0;LevelNames:Off|Switch 1|Switch 2|Both Click", false));
				}
			...
			if ((cmd == "report") || (cmd == "read_ack")) {
				...
				if (ret) {
					...
					if (type != STYPE_END) {
						...
						std::string aqara_wirelesses = root2["dual_channel"].asString();
						...
						else if ((status == "double_click") || (status == "move") || (aqara_wired2 == "on") || (aqara_wirelesses == "both_click")) {
							level = 30;
							on = true;
						}
